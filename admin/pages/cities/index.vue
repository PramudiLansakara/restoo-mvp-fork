<template>
  <v-container fluid>
    <v-data-table :headers="headers" :items="cities" class="elevation-1">
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title><h2>Delivery Cities</h2></v-toolbar-title>
          <v-spacer></v-spacer>
          <v-btn
            :to="'/cities/add'"
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: this.$t("Delivery Charge"), value: "deliveryCharge" },
        { text: this.$t("Minimum Order Price"), value: "minAmount" },
  ...
  async asyncData({ store, error }) {
    try {
      const cities = await store.dispatch("city/getCityList");
      console.log(cities);
      return { cities };
    } catch (err) {
      console.log(err);
  ...
  methods: {
    ...
    deleteItem(item) {
      ...
      this.itemIndex = this.cities.indexOf(item);
    ...
    async deleteItemConfirm() {
      try {
        this.loading = true;
        await this.$store.dispatch("city/deleteCity", this.itemId);
        this.$dialog.message.success("Successfully item deleted!", {
          position: "top-right",
        });
        this.cities.splice(this.itemIndex, 1);
        this.closeDelete();
      } catch (error) {
        this.loading = false;
        console.log(error);
